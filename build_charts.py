...
chart_heatmap = (
    ...
    .properties(width='container', height=450, title='Average Subway Ridership by Day & Precipitation')
...
chart_ridges = (
    ...
    .properties(width=170, height=600,
...
# Chart 3: AQI version 1 (During/Recovery/Post only, removing Pre-COVID)
df_v1 = merged_aqi.copy().sort_values('date')
# Filter out Pre-COVID data (before April 2020)
df_v1 = df_v1[df_v1['date'] >= pd.Timestamp('2020-04-01')].copy()
df_v1['period'] = 'Post-COVID'
df_v1.loc[(df_v1['date'] >= pd.Timestamp('2020-04-01')) & (df_v1['date'] <= pd.Timestamp('2021-06-30')), 'period'] = 'During-COVID'
df_v1.loc[(df_v1['date'] > pd.Timestamp('2021-06-30')) & (df_v1['date'] <= pd.Timestamp('2022-12-31')), 'period'] = 'Recovery'

# Update color scheme without Pre-COVID
periods_v1_filtered = {
    'During-COVID': '#1f77b4',
    'Recovery': '#d62728',
    'Post-COVID': '#ff7f0e'
...
build_aqi_chart(df_v1, periods_v1_filtered, 'COVID period bands', 'aqi_periods')

# Chart 4: AQI version 2 (During/Recovery/Post as in notebook cell 11, removing Pre-Pandemic)
df_v2 = merged_aqi.copy().sort_values('date')
# Filter out Pre-Pandemic data (before April 2020)
df_v2 = df_v2[df_v2['date'] >= pd.Timestamp('2020-04-01')].copy()
df_v2['period'] = df_v2['date'].apply(categorize_period)

# Update color scheme without Pre-Pandemic
periods_v2_filtered = {
...
build_aqi_chart(df_v2, periods_v2_filtered, 'Pandemic timeline', 'aqi_periods_v2')
...
# Filter out data before April 2020 to remove Pre-Pandemic period
tidy_usage = tidy_usage[tidy_usage['date'] >= pd.Timestamp('2020-04-01')].copy()
